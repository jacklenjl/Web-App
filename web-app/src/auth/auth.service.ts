...
export class AuthService {
        ...
        [
          ...
          async (val, cb) => {
            ...
            if (val.length) {
              ...
              const payload = { username: val[0].email, sub: val[0]._id };
              //console.log("payyyloaaad:",payload)
              if (chk) {
  ...
  async findOne(dto: any) {
    let val = await this.userModel.findOne({ _id: dto }).lean();
    console.log('userservice call', dto, val);
    exports.userfinal = val;

    if (val) {
      val = Object.assign(val, { checkUser: 'valid' });
      return val;
    }
    val = Object.assign(val, { checkUser: 'invaild' });
    return val;
  }
}
